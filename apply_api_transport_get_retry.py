#!/usr/bin/env python3
"""
تحسين تجربة المستخدم: إعادة محاولة تلقائية بتأخير تصاعدي لطلبات GET في
ApiTransport عند فشل الشبكة أو انتهاء المهلة — يعالج حالة "cold start"
لسيرفر على استضافة مجانية (Northflank) ينام بعد خمول، بدل رسالة خطأ
حمراء فورية تُجبر المستخدم على الضغط "حاول مرة أخرى" يدويًا.

لا يمسّ POST/PUT إطلاقًا (لا تُعاد محاولتها تلقائيًا أبدًا — تفادي خطر
تكرار الإرسال). محدود لطلبات GET فقط، ولا يُعاد المحاولة عند رد فعلي
من السيرفر (حتى لو 5xx) — فقط عند فشل النقل نفسه (لا استجابة إطلاقًا).

طريقة التشغيل:
  ضعه في أي مكان داخل مستودع نُخبة ثم:
    python3 apply_api_transport_get_retry.py
...
OLD = """  /// The shared request pipeline. Builds the request, applies auth headers,
...
NEW = """  /// Retry budget for idempotent (`GET`) requests only: up to this many
...
def main() -> int:
    ...
    if OLD not in text:
        print("✗ لم يتم العثور على الكتلة المتوقعة — الملف قد يكون تغيّر.")
    ...
    print("  التالي: dart format ثم dart analyze --fatal-infos --fatal-warnings . و dart test packages/api_client")
    return 0
