#!/usr/bin/env bash
# Uploads the 36 UEFA Champions League 2026/27 crest PNGs to the
# `team-logos` Supabase Storage bucket, renamed from their source
# filenames to `<slug>.png` — the exact path ucl_2026_27_crests.sql's
...
#
# Usage:
#   ./upload_ucl_2026_27_crests.sh /path/to/36-png-folder
#
# The folder must contain the 36 files named exactly as the `filename`
# column in ../seed_assets/ucl-2026-27-teams.csv.

set -euo pipefail

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" &> /dev/null && pwd)"
CSV="$SCRIPT_DIR/../seed_assets/ucl-2026-27-teams.csv"
SOURCE_DIR="${1:-}"
