...
base class FakeCompetitionRepository implements CompetitionRepository {
  ...
  // reads over the same backing stores the write methods above use, mirroring
  // the Postgres adapter's join/filter/order semantics — NOT stubs/throws.
  // ---------------------------------------------------------------------------
  ...
  Future<Result<List<OpenRoundFeedEntry>>> listOpenRoundsFeed() async {
    final f = _takeFailure();
    if (f != null) return Result.err(f);
    // Every OPEN round of every PUBLIC competition, joined with the
    // competition's display name (matches the adapter's JOIN), ordered by
    // competition name then id, then round sequence.
    final entries =
        [
          for (final r in _rounds.values)
            if (r.status == RoundStatus.open)
              if (_seasons[r.seasonId.value] case final season?)
