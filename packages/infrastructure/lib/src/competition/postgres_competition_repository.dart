...
''';
  ...
  // queries; see `CompetitionRepository`'s doc for the full contract.
  // --------------------------------------------------------------------------

  // Joins competitions -> seasons -> rounds in one query: every OPEN round of
  // every PUBLIC competition, carrying the competition's display name. Ordered
  // by competition name (then id, tie-break), then round sequence — the same
  // grouping the former client-side walk produced. A corrupt row maps to
  // transient `row_corrupt`, exactly as every other browse read.
  static const String _listOpenRoundsFeedSql = '''
SELECT c.id AS competition_id, c.name AS competition_name,
       r.id AS round_id, r.ruleset_version
...
WHERE r.status = 'open'::competition.round_status
  AND c.visibility = 'public'::competition.visibility
ORDER BY c.name ASC, c.id ASC, r.sequence ASC
''';

  @override
  Future<Result<List<OpenRoundFeedEntry>>> listOpenRoundsFeed() async {
    final result = await _connection.query(_listOpenRoundsFeedSql);
    return switch (result) {
      Err<List<Map<String, dynamic>>>(:final error) => Result.err(error),
      Ok<List<Map<String, dynamic>>>(:final value) => _mapAll(
        value,
        _mapOpenRoundFeedEntry,
      ),
    };
  }
  ...
  Result<OpenRoundFeedEntry> _mapOpenRoundFeedEntry(Map<String, dynamic> row) {
    ...
    if (roundIdResult is Err<RoundId>) {
      return Result.err(
        _corrupt('rounds', 'round_id', roundIdResult.error.message),
      );
    }
  ...
  // The `round_fixtures` analogue of `PostgresFixtureScheduleRepository
  // .findByFixtures`'s `ANY(@ids::uuid[])` batching: every fixture linked to
  // any of [roundIds] in one query, ordered by round (grouping) then
  // display_order (matchday order within each round).
  static const String _listFixturesForRoundsSql = '''
SELECT round_id, fixture_id, display_order
FROM competition.round_fixtures
WHERE round_id = ANY(@round_ids::uuid[])
ORDER BY round_id ASC, display_order ASC, fixture_id ASC
''';
  ...
  Future<Result<List<RoundFixture>>> listFixturesForRounds(
    ...
    if (roundIds.isEmpty) {
      return const Result.ok(<RoundFixture>[]);
    }
    final result = await _connection.query(
      _listFixturesForRoundsSql,
      parameters: {
        'round_ids': [for (final id in roundIds) id.value],
      },
    );
    return switch (result) {
      Err<List<Map<String, dynamic>>>(:final error) => Result.err(error),
