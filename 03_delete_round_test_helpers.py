...
def remove_decl(text: str, signature_marker: str) -> str:
    ...
    if text[end] != ";":
        raise RuntimeError(
            f"لم يُعثر على ';' بعد إغلاق القوس لتصريح: {signature_marker!r} "
            f"(وجد: {text[end:end + 30]!r})"
        )
    end += 1
    if text[end : end + 1] == "\n":
        end += 1
...
ledger_text = remove_decl(ledger_text, "Round roundIn(RoundStatus status)")
ledger_text = remove_decl(
    ledger_text, "RoundScore storedScore(String participantId, int total)"
)
...
scoring_text = remove_decl(scoring_text, "Round roundIn(RoundStatus status)")
scoring_text = remove_decl(
    scoring_text, "Participant participant(String id, String userId)"
)
scoring_text = remove_decl(
    scoring_text, "RoundFixture link(String fixtureId, int order)"
)
scoring_text = remove_decl(
    scoring_text,
    "FixtureResult actualResult(String fixtureId, int home, int away)",
)
scoring_text = remove_decl(scoring_text, "Prediction prediction({")

ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
backup_dir = pathlib.Path.home() / "nukhbaa-round-backups" / f"server_test_helpers_{ts}"
...
log_line = (
    f"- [{now}] تنظيف: حذف عناصر مساعدة يتيمة في ملفي اختبار السيرفر بعد حذف مجموعات "
    f"اختبار Round (السكربت 02) — ledger_routes_test.dart: roundIn/storedScore؛ "
    f"scoring_routes_test.dart: kParticipantId/kPredictionId/roundIn/participant/"
    f"link/actualResult/prediction | "
...
subprocess.run(
    ["git", "commit", "-m", "test(server): remove now-unused Round test helpers in ledger/scoring route tests"],
    check=True,
)
