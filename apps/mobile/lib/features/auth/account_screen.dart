...
class AccountScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Nukhba'), actions: <Widget>[
        IconButton(
          ...
          icon: Badge(
            ...
            label: unread.maybeWhen(data: (count) => count > 0 ? Text('$count') : null, orElse: () => null),
            isLabelVisible: unread.maybeWhen(data: (count) => count > 0, orElse: () => false),
            child: const Icon(Icons.notifications_outlined),
          ),
        ),
        IconButton(key: const Key('account.signOut'), tooltip: 'Sign out', icon: const Icon(Icons.logout),
        ...
        child: Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 480),
          child: SingleChildScrollView(padding: const EdgeInsets.all(24),
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: <Widget>[
              const Text('Signed in', key: Key('account.title'), style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
              const SizedBox(height: 16),
              _Field(label: 'User ID', value: user.userId, valueKey: const Key('account.userId')),
              _Field(label: 'Role', value: user.role, valueKey: const Key('account.role')),
              _Field(label: 'Status', value: user.status, valueKey: const Key('account.status')),
              if (user.email != null) _Field(label: 'Email', value: user.email!, valueKey: const Key('account.email')),
              const SizedBox(height: 24),
              FilledButton.icon(
                key: const Key('account.browseCompetitions'),
                icon: const Icon(Icons.emoji_events_outlined),
                label: const Text('Browse competitions'),
                onPressed: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const CompetitionListScreen())),
              ),
              ...
              OutlinedButton.icon(
                ...
                onPressed: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const HallOfFameScreen())),
              ),
              const SizedBox(height: 12),
              OutlinedButton.icon(
                ...
                onPressed: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const PredictionHistoryScreen())),
              ),
              const SizedBox(height: 12),
              OutlinedButton.icon(
                ...
                onPressed: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const CreateGroupScreen())),
              ),
              const SizedBox(height: 12),
              OutlinedButton.icon(
                ...
                onPressed: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const JoinGroupScreen())),
              ),
              ...
              if (user.role == 'admin') ...<Widget>[
                ...
                OutlinedButton.icon(
                  ...
                  onPressed: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const AdminDashboardScreen())),
                ),
              ],
            ]),
          ),
...
class _Field extends StatelessWidget {
  const _Field({required this.label, required this.value, required this.valueKey});
  final String label;
  final String value;
  final Key valueKey;
  @override
  Widget build(BuildContext context) {
    return Padding(padding: const EdgeInsets.symmetric(vertical: 8),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
        Text(label, style: const TextStyle(fontWeight: FontWeight.w600, color: AppColors.textSecondary, fontSize: 12)),
        ...
        Container(width: double.infinity, padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12),
          ...
          child: Text(value, key: valueKey, style: const TextStyle(color: AppColors.textPrimary))),
      ]),
    );
  }
}