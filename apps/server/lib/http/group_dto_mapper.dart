...
/// Shapes the response of `GET /me/groups` — every group the caller belongs
/// to, each row built with the same [groupToDto] projection every other
/// member-visible group read uses (invite code included — the caller is a
/// member of every row here by construction), paired with the caller's own
/// role and when they joined. The [summaries] order (joined-at descending) is
/// passed straight through — this mapper reorders nothing.
Map<String, Object?> myGroupsJson(List<MyGroupSummary> summaries) {
  return MyGroupsDto(
    groups: [
      for (final s in summaries)
        MyGroupEntryDto(
          group: groupToDto(s.group, memberCount: s.memberCount),
          role: s.role.wireValue,
          joinedAt: s.joinedAt.toUtc().toIso8601String(),
        ),
    ],
  ).toJson();
}
