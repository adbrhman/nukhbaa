...
final class InMemoryCompetitionRepository implements CompetitionRepository {
  ...
  Future<Result<List<CompetitionSeason>>> listMonthlySeasons() async {
    final monthly = [
      for (final s in seasons.values)
        if (RegExp(r'^[0-9]{2}/[0-9]{4}$').hasMatch(s.label)) s,
...
final class InMemoryLeaderboardRepository implements LeaderboardRepository {
  ...
  List<HallOfFameEntry> _allTime = const [];
  ...
  /// Seeds the unranked all-time (Hall of Fame) [entries].
...
final class InMemoryGroupRepository implements GroupRepository {
  ...
  Future<Result<List<MyGroupSummary>>> listGroupsForUser(UserId userId) async {
    ...
    if (f != null) return Result.err(f);
    final own =
        memberships.where((m) => m.userId.value == userId.value).toList()
          ..sort((a, b) => b.joinedAt.compareTo(a.joinedAt));
    final list = <MyGroupSummary>[];
    for (final m in own) {
      final group = groups[m.groupId.value];
      if (group == null) continue;
      final memberCount = memberships
          .where((x) => x.groupId.value == m.groupId.value)
          .length;
      list.add(
        MyGroupSummary(
          group: group,
